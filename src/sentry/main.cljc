(ns sentry.main
  "Kotodama WASM entrypoint for the Sentry clean-room actor (L4) — Clojure port.
  ...
  facts (`sentry.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "sentry")
(def tier "L4")
...
(def entity-specs
  [{:entity "Issue"        :plural "issues"        :id-prefix "sentry_iss"
    :fields [:shortId :title :status :level :priority :count :userCount] :required [:title :status]
    :coerce {:count :int :userCount :int}   :refs {}}
   {:entity "Event"        :plural "events"        :id-prefix "sentry_eve"
    :fields [:eventID :groupID :projectID :title :message :level :platform] :required [:eventID :groupID]
    :coerce {}                      :refs {}}
   {:entity "Project"      :plural "projects"      :id-prefix "sentry_pro"
    :fields [:slug :name :platform :isBookmarked] :required [:slug :name]
    :coerce {:isBookmarked :bool}   :refs {}}
   {:entity "Organization" :plural "organizations" :id-prefix "sentry_org"
    :fields [:slug :name :hasAuthProvider :require2FA :isEarlyAdopter] :required [:slug :name]
    :coerce {:hasAuthProvider :bool :require2FA :bool :isEarlyAdopter :bool} :refs {}}
   {:entity "Team"         :plural "teams"         :id-prefix "sentry_tea"
    :fields [:slug :name :memberCount :hasAccess] :required [:slug :name]
    :coerce {:memberCount :int :hasAccess :bool} :refs {}}
   {:entity "Release"      :plural "releases"      :id-prefix "sentry_rel"
    :fields [:version :status :commitCount :deployCount] :required [:version :status]
    :coerce {:commitCount :int :deployCount :int} :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"sentry.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "sentry-compat" :tier tier :entities entities} 200])
...
(defn start! [] :sentry-compat/ready)
